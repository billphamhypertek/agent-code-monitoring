    ...
    embeds: [
      {
        ...
        color: ACCENT_INT,
        fields,
        footer: { text: "Agent Code Monitoring" },
...
function formatPagerDuty(alert, config) {
  return {
    ...
    dedup_key: `acm:${alert.rule_id || "test"}:${alert.session_id || ""}`,
    payload: {
      summary: truncate(`${alert.rule_name}: ${alert.message}`, 1024),
      source: alert.session_id || "agent-code-monitoring",
...
function formatOpsgenie(alert) {
  return {
    ...
    alias: `acm:${alert.rule_id || "test"}:${alert.session_id || ""}`,
    description: truncate(alert.message, 15000),
    source: "agent-code-monitoring",
...
function formatSplunkOnCall(alert, config) {
  return {
    ...
    entity_id: `acm:${alert.rule_id || "test"}:${alert.session_id || ""}`,
    ...
    monitoring_tool: "agent-code-monitoring",
  };
}
