...
function writeFixtureDir() {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), "acm-fixture-"));
...
describe("POST /api/import/scan-path validation", () => {
  ...
  it("rejects non-existent paths", async () => {
    const res = await post("/api/import/scan-path", {
      path: "/definitely/does/not/exist/acm-" + Date.now(),
    });
    assert.equal(res.status, 400);
  ...
  it("rejects files (not directories)", async () => {
    const tmp = path.join(os.tmpdir(), `acm-not-dir-${Date.now()}.txt`);
...
describe("archive helpers", () => {
  it("isPathInside rejects traversal", () => {
    const root = path.resolve("/tmp/acm-root");
    assert.equal(archive.isPathInside(root, "/tmp/acm-root/ok.jsonl"), true);
    assert.equal(archive.isPathInside(root, "/tmp/other/bad.jsonl"), false);
    assert.equal(archive.isPathInside(root, "/tmp/acm-root/../escape"), false);
  });
  ...
  it("extractGzSingle decompresses plain gz", async () => {
    const dest = fs.mkdtempSync(path.join(os.tmpdir(), "acm-gz-"));
...
describe("importFromDirectory directly", () => {
  it("reports progress and never throws on empty dirs", async () => {
    const empty = fs.mkdtempSync(path.join(os.tmpdir(), "acm-empty-"));
    try {
      const events = [];
  ...
  it("matches the legacy importer's token totals on the same fixtures", async () => {
    ...
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "acm-fresh-"));
  ...
  it("snapshots the transcript into the data dir so it survives Claude Code pruning", async () => {
    ...
    const dataDir = fs.mkdtempSync(path.join(os.tmpdir(), "acm-data-"));
    process.env.DASHBOARD_DATA_DIR = dataDir;
    const src = fs.mkdtempSync(path.join(os.tmpdir(), "acm-src-"));
...
describe("tar path-traversal hardening", () => {
  it("extractTar rejects entries with ../ segments", async () => {
    ...
    const tmp = fs.mkdtempSync(path.join(os.tmpdir(), "acm-tar-bad-"));
...
describe("extraction size cap", () => {
  it("extractGzSingle aborts past MAX_EXTRACT_BYTES", async () => {
    const prev = process.env.ACM_IMPORT_MAX_EXTRACT_BYTES;
    process.env.ACM_IMPORT_MAX_EXTRACT_BYTES = "128";
    ...
    const tmp = fs.mkdtempSync(path.join(os.tmpdir(), "acm-bomb-"));
    ...
    try {
      ...
      fs.rmSync(tmp, { recursive: true, force: true });
      if (prev === undefined) delete process.env.ACM_IMPORT_MAX_EXTRACT_BYTES;
      else process.env.ACM_IMPORT_MAX_EXTRACT_BYTES = prev;
...
describe("orphan subagent inference", () => {
  it("attaches subagent via Layout 2: <proj>/subagents/<sessionId>/agent.jsonl", async () => {
    ...
    const seedRoot = fs.mkdtempSync(path.join(os.tmpdir(), "acm-orphan-seed-"));
    ...
    const orphanRoot = fs.mkdtempSync(path.join(os.tmpdir(), "acm-orphan-"));
...
describe("concurrent scan-path requests", () => {
  it("two concurrent imports of different folders both succeed without clobbering", async () => {
    ...
    const rootA = fs.mkdtempSync(path.join(os.tmpdir(), "acm-concurrent-a-"));
    const rootB = fs.mkdtempSync(path.join(os.tmpdir(), "acm-concurrent-b-"));
