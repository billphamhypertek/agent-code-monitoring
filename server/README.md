...
Enterprise-grade Node.js backend for Agent Code Monitoringing with real-time WebSocket updates.
...
sequenceDiagram
    ...
    R->>A: rmTempDir(workDir + req._acmUploadDir)
```
...
| --------------------------------- | ----------- | ----------------------------------------------------------------- |
| `ACM_IMPORT_MAX_BYTES`           | `1073741824` | Maximum size per uploaded file                                   |
| `ACM_IMPORT_MAX_FILES`           | `2000`      | Maximum files per upload request                                 |
| `ACM_IMPORT_MAX_EXTRACT_BYTES`   | `4294967296` | Total uncompressed bytes allowed per archive (zip-bomb guard)   |
...
```bash
# Build and run
docker build -t agent-code-monitoring .
docker run -p 4820:4820 -v $(pwd)/data:/app/data agent-code-monitoring
```
