...
function createApp() {
  ...
  app.use(
    ...
    swaggerUi.setup(openApiSpec, {
      customSiteTitle: "Agent Code Monitoring API Docs",
    })
  );
...
function startServer(app, port) {
  ...
  return new Promise((resolve) => {
    server.listen(port, () => {
      ...
      console.log(`Agent Code Monitoring server running on http://localhost:${port} (${mode})`);
      if (!isProduction) {
...
if (require.main === module) {
  ...
  probeDashboardHealth(PORT).then((alreadyRunning) => {
    if (alreadyRunning && !isWatchMode) {
      console.log(
        `Agent Code Monitoring is already running on http://localhost:${PORT} — not starting a ` +
