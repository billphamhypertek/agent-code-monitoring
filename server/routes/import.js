...
const MAX_UPLOAD_BYTES = parseInt(
  process.env.ACM_IMPORT_MAX_BYTES || String(1024 * 1024 * 1024), // 1 GB default
  10
);
const MAX_UPLOAD_FILES = parseInt(process.env.ACM_IMPORT_MAX_FILES || "2000", 10);
...
function getUploader() {
  ...
  const storage = multer.diskStorage({
    destination: (req, _file, cb) => {
      if (!req._acmUploadDir) req._acmUploadDir = mkTempDir("acm-upload-");
      cb(null, req._acmUploadDir);
    },
  ...
  return multer({
    ...
    fileFilter: (req, file, cb) => {
      ...
      if (kind === "unknown") {
        ...
        if (!req._acmRejected) req._acmRejected = [];
        req._acmRejected.push(file.originalname);
        cb(null, false);
      } else {
        cb(null, true);
      }
...
router.post("/upload", uploadMiddleware, async (req, res) => {
  ...
  const rejectedNames = Array.isArray(req._acmRejected) ? req._acmRejected : [];
  const reqUploadDir = req._acmUploadDir || null;
  ...
  const workDir = mkTempDir("acm-import-work-");
