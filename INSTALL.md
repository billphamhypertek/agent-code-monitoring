...
A step-by-step guide to get the Agent Code Monitoring up and running on your machine, with optional sections for importing history, running in a container, and using the native desktop app (macOS & Windows).
...
| `ACM_IMPORT_MAX_EXTRACT_BYTES`   | 4 GB    | Ceiling on uncompressed bytes per archive (zip-bomb guard)  |

Set them before `npm run dev` or `npm start`:

```bash
ACM_IMPORT_MAX_EXTRACT_BYTES=17179869184 npm start   # allow 16 GB extraction
```
...
Open [**Releases → latest**](https://github.com/billphamhypertek/agent-code-monitoring/releases/latest) and download the asset for your platform. CI publishes a new `vX.Y.Z` release automatically every time the version in `package.json` is bumped on `master`, so this link always points at the current shipping build.
...
The **Agent Code Monitoring** is also available as a dedicated VS Code extension for seamless, integrated monitoring.
