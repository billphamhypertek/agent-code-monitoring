...
Enterprise deployment strategies for Agent Code Monitoring across development, staging, and production environments.
...
Agent Code Monitoring supports multiple deployment modes:
...
```bash
# Clone repository
git clone https://github.com/your-org/agent-code-monitoring.git
...
export PORT=4820
export DASHBOARD_DB_PATH=/var/lib/agent-code-monitoring/dashboard.db

# 4. Create data directory
mkdir -p /var/lib/agent-code-monitoring
...
# Database
DASHBOARD_DB_PATH=/var/lib/agent-code-monitoring/dashboard.db
...
# Initialize
eb init -p node.js agent-code-monitoring
...
kind: Deployment
metadata:
  name: agent-code-monitoring
spec:
  replicas: 3
  selector:
    matchLabels:
      app: agent-code-monitoring
  template:
    metadata:
      labels:
        app: agent-code-monitoring
    spec:
      containers:
      - name: agent-code-monitoring
        image: agent-code-monitoring:latest
        ports:
        - containerPort: 4820
        env:
        - name: NODE_ENV
      ...
      - name: data
        persistentVolumeClaim:
          claimName: agent-code-monitoring-pvc
...
graph TB
    subgraph "Kubernetes Cluster"
        ...
        subgraph "Pods"
            Pod1[agent-code-monitoring-1<br/>:4820]
            Pod2[agent-code-monitoring-2<br/>:4820]
            Pod3[agent-code-monitoring-3<br/>:4820]
        end
...
# Start application
pm2 start server/index.js --name agent-code-monitoring

# Start with environment
pm2 start server/index.js --name agent-code-monitoring --env production

# View logs
pm2 logs agent-code-monitoring

# Monitor
pm2 monit

# Restart
pm2 restart agent-code-monitoring

# Stop
pm2 stop agent-code-monitoring
...
module.exports = {
  apps: [{
    ...
    error_file: '/var/log/agent-code-monitoring/error.log',
    out_file: '/var/log/agent-code-monitoring/out.log',
    time: true
  }]
};
```
...
```ini
# /etc/systemd/system/agent-code-monitoring.service
[Unit]
Description=Agent Code Monitoring
After=network.target

[Service]
Type=simple
User=agent-code-monitoring
WorkingDirectory=/opt/agent-code-monitoring
Environment=NODE_ENV=production
Environment=PORT=4820
Environment=DASHBOARD_DB_PATH=/var/lib/agent-code-monitoring/dashboard.db
...
sudo systemctl enable agent-code-monitoring
sudo systemctl start agent-code-monitoring

# Check status
sudo systemctl status agent-code-monitoring

# View logs
sudo journalctl -u agent-code-monitoring -f
```
...
BACKUP_DIR="/var/backups/agent-code-monitoring"
DB_PATH="/var/lib/agent-code-monitoring/dashboard.db"
...
aws s3 cp "$BACKUP_FILE.gz" s3://my-backups/agent-code-monitoring/
...
# Stop application
systemctl stop agent-code-monitoring
...
chown agent-code-monitoring:agent-code-monitoring "$DB_PATH"

# Start application
systemctl start agent-code-monitoring
...
```nginx
# /etc/nginx/sites-available/agent-code-monitoring
server {
