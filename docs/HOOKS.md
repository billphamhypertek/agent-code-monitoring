...
Claude Code provides a hook system that allows external tools to receive real-time events during agent execution. Agent Code Monitoring uses these hooks to capture session lifecycle, tool executions, and notifications.
...
> 2. Otherwise it reads `~/.claude/.agent-code-monitoring.json`, a JSON document
