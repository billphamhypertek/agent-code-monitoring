...
The Agent Code Monitoring MCP server exposes dashboard functionality as tools that can be used by Claude Desktop, Cline, and other MCP clients.
...
graph TB
    ...
    subgraph "MCP Server"
        MCPServer[Agent Code Monitoring<br/>MCP Server]
        Tools[Tool Registry]
    end
...
```json
{
  "mcpServers": {
    "agent-code-monitoring": {
      "command": "node",
      "args": ["/path/to/agent-code-monitoring/mcp/dist/index.js"],
      "env": {
...
# Run container
docker run -e MCP_DASHBOARD_BASE_URL=http://localhost:4820 agent-code-monitoring-mcp:local
```
...
# Run with Podman
podman run -e MCP_DASHBOARD_BASE_URL=http://localhost:4820 localhost/agent-code-monitoring-mcp:local
```
