...
Comprehensive database schema documentation for Agent Code Monitoring SQLite database.
...
Agent Code Monitoring uses **SQLite 3** as its primary data store with the following characteristics:
...
- Production: `/var/lib/agent-code-monitoring/dashboard.db` (configurable via `DASHBOARD_DB_PATH`)
...
# Copy database file
cp /var/lib/agent-code-monitoring/dashboard.db /backups/dashboard_$(date +%Y%m%d).db
