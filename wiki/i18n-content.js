...
window.__WIKI_CONTENT_I18N = {
  zh: {
    ...
    "Agent Code Monitoring integrates with Claude Code through its native hook system. When Claude Code performs any action — tool use, session start, subagent orchestration, session end — it fires a hook that calls a small Node.js script bundled with this project. That script forwards the event over HTTP to the dashboard server, which stores it in SQLite and broadcasts it to the browser over WebSocket.":
      "Agent Code Monitoring 通过 Claude Code 的原生 hook 系统与之集成。当 Claude Code 执行任何操作时——工具使用、会话开始、subagent 编排、会话结束——它都会触发一个 hook，调用本项目附带的一个小型 Node.js 脚本。该脚本通过 HTTP 将事件转发给仪表盘服务器，服务器将其存入 SQLite 并通过 WebSocket 广播到浏览器。",
    ...
    "This chart tracks how interest in Agent Code Monitoring has grown over time. The curve keeps climbing as more developers discover the project, share it, and use it in real workflows. Each new star is a small vote of confidence from the community.":
      "此图表追踪人们对 Agent Code Monitoring 的关注度如何随时间增长。随着越来越多的开发者发现该项目、分享它并在真实工作流中使用它，曲线持续攀升。每一颗新的 star 都是社区投出的一份小小的信任票。",
    '<span class="caption-icon">⭐</span> <span> Enjoying the project? <a class="star-history-caption-link" href="https://github.com/billphamhypertek/agent-code-monitoring" target="_blank" rel="noopener noreferrer" >Give it a star on GitHub</a > and help more builders discover it. </span>':
      '<span class="caption-icon">⭐</span> <span> 喜欢这个项目吗？ <a class="star-history-caption-link" href="https://github.com/billphamhypertek/agent-code-monitoring" target="_blank" rel="noopener noreferrer" >在 GitHub 上给它点个 star</a >，帮助更多开发者发现它。 </span>',
    "Hook Type": "Hook 类型",
    Trigger: "触发条件",
    ...
    "Build MCP container image with Docker (<code>agent-code-monitoring-mcp:local</code>)":
      "使用 Docker 构建 MCP 容器镜像（<code>agent-code-monitoring-mcp:local</code>）",
    "Build MCP container image with Podman (<code>localhost/agent-code-monitoring-mcp:local</code>)":
      "使用 Podman 构建 MCP 容器镜像（<code>localhost/agent-code-monitoring-mcp:local</code>）",
    ...
    "Build the Windows NSIS installer <code>AgentCodeMonitoring-Setup-&lt;ver&gt;-x64.exe</code> (Windows host)":
      "构建 Windows NSIS 安装程序 <code>AgentCodeMonitoring-Setup-&lt;ver&gt;-x64.exe</code>（Windows 主机）",
    "Build the no-install portable <code>AgentCodeMonitoring-&lt;ver&gt;-x64-portable.exe</code> (Windows host)":
      "构建免安装的便携版 <code>AgentCodeMonitoring-&lt;ver&gt;-x64-portable.exe</code>（Windows 主机）",
    ...
    "Per-request temp dir on <code>req._acmUploadDir</code>; created in multer <code>destination</code>, reclaimed in <code>finally</code>":
      "在 <code>req._acmUploadDir</code> 上为每个请求设置临时目录；在 multer <code>destination</code> 中创建，在 <code>finally</code> 中回收",
    ...
    '📖 Full documentation: <a href="https://github.com/billphamhypertek/agent-code-monitoring/blob/master/docs/PLUGINS.md"><code>docs/plugins.md</code></a>':
      '📖 完整文档：<a href="https://github.com/billphamhypertek/agent-code-monitoring/blob/master/docs/PLUGINS.md"><code>docs/plugins.md</code></a>',
    ...
    "The <b>Agent Code Monitoring</b> is a premium, high-fidelity extension designed to minimize context switching for AI engineers. It brings the full power of the dashboard directly into VS Code, allowing you to monitor complex subagent orchestration without ever leaving your active code file.":
      "<b>Agent Code Monitoring</b> 是一款高品质、高保真的扩展，旨在为 AI 工程师减少上下文切换。它将仪表盘的全部能力直接带入 VS Code，让你无需离开当前代码文件即可监控复杂的子代理编排。",
    ...
    '📖 Full developer guide: <a href="https://github.com/billphamhypertek/agent-code-monitoring/blob/master/vscode-extension/README.md"><code>vscode-extension/README.md</code></a>':
      '📖 完整开发者指南：<a href="https://github.com/billphamhypertek/agent-code-monitoring/blob/master/vscode-extension/README.md"><code>vscode-extension/README.md</code></a>',
    ...
    '<span class="caption-icon">🍎🪟</span> <span>The full dashboard, natively on macOS <strong>and</strong> Windows — same React client, same Express server, real <code>BrowserWindow</code>. Menu-bar / notification-area (tray) icon included. Shipped as a macOS DMG and a Windows EXE (macOS shown) — see <a href="https://github.com/billphamhypertek/agent-code-monitoring/blob/master/DESKTOP.md"><code>DESKTOP.md</code></a>.</span>':
      '<span class="caption-icon">🍎🪟</span> <span>完整的仪表盘，原生运行于 macOS <strong>和</strong> Windows — 相同的 React 客户端，相同的 Express 服务器，真正的 <code>BrowserWindow</code>。包含菜单栏 / 通知区域（托盘）图标。以 macOS DMG 和 Windows EXE 形式发布（图中所示为 macOS） — 参见 <a href="https://github.com/billphamhypertek/agent-code-monitoring/blob/master/DESKTOP.md"><code>DESKTOP.md</code></a>。</span>',
    ...
    "Launch the desktop app and <code>npm run dev</code> at the same time and both stay real-time. Each server appends its <code>{port, pid, startedAt}</code> entry to <code>~/.claude/.agent-code-monitoring.json</code> on startup; the Claude Code hook handler reads that list and fan-outs every event to every live entry in parallel. Stale entries self-evict via a PID liveness check on read, so a crashed server can't misroute events to a dead port.":
      "同时启动桌面应用和 <code>npm run dev</code>，两者都能保持实时。每个服务器在启动时都会将其 <code>{port, pid, startedAt}</code> 条目追加到 <code>~/.claude/.agent-code-monitoring.json</code>；Claude Code 的 hook 处理程序读取该列表，并将每个事件并行扇出到每个存活的条目。陈旧的条目会在读取时通过 PID 存活检查自行清除，因此崩溃的服务器无法将事件误路由到已失效的端口。",
    ...
    "Run <code>AgentCodeMonitoring-Setup-&lt;ver&gt;-x64.exe</code> — a per-user NSIS install (no admin), or run the <code>*-portable.exe</code> to skip installing":
      "运行 <code>AgentCodeMonitoring-Setup-&lt;ver&gt;-x64.exe</code>——按用户的 NSIS 安装（无需管理员），或运行 <code>*-portable.exe</code> 以跳过安装",
    ...
    '<span class="caption-icon">2️⃣</span> <span>NSIS installer, step 2 — <strong>Choose Install Location</strong>: defaults to <code>%LOCALAPPDATA%\\Programs\\Agent Code Monitoring</code>, or point it anywhere.</span>':
      '<span class="caption-icon">2️⃣</span> <span>NSIS 安装程序，第 2 步——<strong>Choose Install Location</strong>：默认为 <code>%LOCALAPPDATA%\\Programs\\Agent Code Monitoring</code>，也可指向任意位置。</span>',
    ...
    'The installer and portable EXE are <strong>unsigned</strong> by default — that is all the project can offer without a paid code-signing certificate. Windows <strong>SmartScreen</strong> may show <em>"Windows protected your PC"</em> the first time you run it; click <strong>More info → Run anyway</strong>. The installer lays the app down <strong>per-user</strong> under <code>%LOCALAPPDATA%\\Programs\\Agent Code Monitoring</code> (and lets you choose the install directory) and sets an <code>AppUserModelId</code> (<code>com.billphamhypertek.acm.desktop</code>) so native toast notifications are attributed correctly and the window groups under one taskbar entry.':
      "安装程序和便携版 EXE 默认<strong>未签名</strong>——在没有付费代码签名证书的情况下，这已是该项目所能提供的全部。Windows <strong>SmartScreen</strong> 可能会在你首次运行时显示 <em>“Windows protected your PC”</em>；点击 <strong>More info → Run anyway</strong>。安装程序会将应用<strong>按用户</strong>放置在 <code>%LOCALAPPDATA%\\Programs\\Agent Code Monitoring</code> 下（并允许你选择安装目录），并设置一个 <code>AppUserModelId</code>（<code>com.billphamhypertek.acm.desktop</code>），以便原生的 toast 通知能被正确归属，且窗口会归入同一个任务栏条目下。",
    "Bundle size": "包大小",
    "The DMG is roughly 80&nbsp;MB, about 250&nbsp;MB installed on disk — the standard Electron tax; the Windows installer is comparable. The app runs natively on <strong>macOS and Windows</strong>; Linux is tracked as a follow-up. Logs live at <code>~/Library/Logs/Agent Code Monitoring/desktop.log</code> on macOS or <code>%APPDATA%\\Agent Code Monitoring\\logs\\desktop.log</code> on Windows (reach them from the tray menu → <em>Show Logs</em>).":
      "DMG 大约 80&nbsp;MB，安装到磁盘上约 250&nbsp;MB——这是标准的 Electron 代价；Windows 安装程序也相当。该应用在 <strong>macOS 和 Windows</strong> 上原生运行；Linux 作为后续项被跟踪。日志在 macOS 上位于 <code>~/Library/Logs/Agent Code Monitoring/desktop.log</code>，在 Windows 上位于 <code>%APPDATA%\\Agent Code Monitoring\\logs\\desktop.log</code>（可从托盘菜单 → <em>Show Logs</em> 找到它们）。",
    '📖 User-facing guide: <a href="https://github.com/billphamhypertek/agent-code-monitoring/blob/master/DESKTOP.md"><code>DESKTOP.md</code></a> · architecture &amp; contributor reference: <a href="https://github.com/billphamhypertek/agent-code-monitoring/blob/master/desktop/README.md"><code>desktop/README.md</code></a>':
      '📖 面向用户的指南：<a href="https://github.com/billphamhypertek/agent-code-monitoring/blob/master/DESKTOP.md"><code>DESKTOP.md</code></a> · 架构与贡献者参考：<a href="https://github.com/billphamhypertek/agent-code-monitoring/blob/master/desktop/README.md"><code>desktop/README.md</code></a>',
    ...
    '<span class="caption-icon">⭐</span> <span> Enjoying the project? <a class="star-history-caption-link" href="https://github.com/billphamhypertek/agent-code-monitoring" target="_blank" rel="noopener noreferrer">Give it a star on GitHub</a> and help more builders discover it. </span>':
      '<span class="caption-icon">⭐</span> <span> 喜欢这个项目吗？<a class="star-history-caption-link" href="https://github.com/billphamhypertek/agent-code-monitoring" target="_blank" rel="noopener noreferrer">在 GitHub 上给它一个 star</a>，帮助更多开发者发现它。 </span>',
    'Clears the waiting flag and promotes the main agent to <span class="status-chip chip-working"><span class="chip-dot"></span>Working</span>. The only reliable signal that text-only assistant turns have started — they emit no <code>PreToolUse</code> before <code>Stop</code>.':
    ...
    'Want a build straight off the tip of <code>master</code>, ahead of the next tagged release? Every green run of the <code>🍎 macOS Desktop (DMG)</code> job on <code>macos-latest</code> uploads the universal DMG as the <code>AgentCodeMonitoring-dmg</code> workflow artifact, and the <code>🪟 Windows Desktop (EXE)</code> job on <code>windows-latest</code> uploads the installer + portable EXEs as the <code>AgentCodeMonitoring-win</code> artifact. Open the <a href="https://github.com/billphamhypertek/agent-code-monitoring/actions/workflows/ci.yml?query=branch%3Amaster+is%3Asuccess" target="_blank" rel="noopener noreferrer" class="dl-chip"> <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><circle cx="12" cy="12" r="9"></circle><polyline points="12 7 12 12 15 14"></polyline></svg> latest passing run </a>, scroll to its Artifacts section, and download <code>AgentCodeMonitoring-dmg</code> or <code>AgentCodeMonitoring-win</code>. (GitHub sign-in required; 14-day retention.)':
      '想要直接从 <code>master</code> 最新提交构建、领先于下一个打标签的发布版本？<code>macos-latest</code> 上 <code>🍎 macOS Desktop (DMG)</code> 任务的每一次绿色运行都会将通用 DMG 作为 <code>AgentCodeMonitoring-dmg</code> 工作流工件上传，而 <code>windows-latest</code> 上的 <code>🪟 Windows Desktop (EXE)</code> 任务会将安装程序 + 便携式 EXE 作为 <code>AgentCodeMonitoring-win</code> 工件上传。打开 <a href="https://github.com/billphamhypertek/agent-code-monitoring/actions/workflows/ci.yml?query=branch%3Amaster+is%3Asuccess" target="_blank" rel="noopener noreferrer" class="dl-chip"> <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><circle cx="12" cy="12" r="9"></circle><polyline points="12 7 12 12 15 14"></polyline></svg> latest passing run </a>，滚动到其 Artifacts 部分，下载 <code>AgentCodeMonitoring-dmg</code> 或 <code>AgentCodeMonitoring-win</code>。（需要登录 GitHub；保留 14 天。）',
  ...
  vi: {
    ...
    "This chart tracks how interest in Agent Code Monitoring has grown over time. The curve keeps climbing as more developers discover the project, share it, and use it in real workflows. Each new star is a small vote of confidence from the community.":
      "Biểu đồ này theo dõi mức độ quan tâm đến Agent Code Monitoring đã tăng trưởng như thế nào theo thời gian. Đường cong tiếp tục đi lên khi ngày càng nhiều nhà phát triển khám phá dự án, chia sẻ nó và dùng nó trong các quy trình làm việc thực tế. Mỗi star mới là một lá phiếu tin tưởng nhỏ từ cộng đồng.",
    '<span class="caption-icon">⭐</span> <span> Enjoying the project? <a class="star-history-caption-link" href="https://github.com/billphamhypertek/agent-code-monitoring" target="_blank" rel="noopener noreferrer" >Give it a star on GitHub</a > and help more builders discover it. </span>':
      '<span class="caption-icon">⭐</span> <span> Bạn thích dự án này? <a class="star-history-caption-link" href="https://github.com/billphamhypertek/agent-code-monitoring" target="_blank" rel="noopener noreferrer" >Hãy gắn sao cho nó trên GitHub</a > và giúp nhiều nhà phát triển khác khám phá nó. </span>',
    "Hook Type": "Loại Hook",
    ...
    "Build MCP container image with Docker (<code>agent-code-monitoring-mcp:local</code>)":
      "Build image container MCP bằng Docker (<code>agent-code-monitoring-mcp:local</code>)",
    "Build MCP container image with Podman (<code>localhost/agent-code-monitoring-mcp:local</code>)":
      "Build image container MCP bằng Podman (<code>localhost/agent-code-monitoring-mcp:local</code>)",
    ...
    "Build the Windows NSIS installer <code>AgentCodeMonitoring-Setup-&lt;ver&gt;-x64.exe</code> (Windows host)":
      "Build trình cài đặt Windows NSIS <code>AgentCodeMonitoring-Setup-&lt;ver&gt;-x64.exe</code> (host Windows)",
    "Build the no-install portable <code>AgentCodeMonitoring-&lt;ver&gt;-x64-portable.exe</code> (Windows host)":
      "Build phiên bản portable không cần cài <code>AgentCodeMonitoring-&lt;ver&gt;-x64-portable.exe</code> (host Windows)",
    "Regenerate <code>desktop/assets/icon.ico</code> from <code>icon.png</code> (PowerShell + .NET; Windows host)":
    ...
    "Per-request temp dir on <code>req._acmUploadDir</code>; created in multer <code>destination</code>, reclaimed in <code>finally</code>":
      "Thư mục tạm cho mỗi yêu cầu trên <code>req._acmUploadDir</code>; được tạo trong multer <code>destination</code>, được thu hồi trong <code>finally</code>",
    ...
    '📖 Full documentation: <a href="https://github.com/billphamhypertek/agent-code-monitoring/blob/master/docs/PLUGINS.md"><code>docs/plugins.md</code></a>':
      '📖 Tài liệu đầy đủ: <a href="https://github.com/billphamhypertek/agent-code-monitoring/blob/master/docs/PLUGINS.md"><code>docs/plugins.md</code></a>',
    '<span class="caption-icon">🖥️</span> Statusline — always-visible bar showing context window usage, token counts, active model, git branch, and session ID. Configurable segments with theme support':
    ...
    "The <b>Agent Code Monitoring</b> is a premium, high-fidelity extension designed to minimize context switching for AI engineers. It brings the full power of the dashboard directly into VS Code, allowing you to monitor complex subagent orchestration without ever leaving your active code file.":
      "<b>Agent Code Monitoring</b> là một tiện ích mở rộng cao cấp, độ trung thực cao được thiết kế để giảm thiểu việc chuyển đổi ngữ cảnh cho các kỹ sư AI. Nó đưa toàn bộ sức mạnh của bảng điều khiển trực tiếp vào VS Code, cho phép bạn giám sát việc điều phối subagent phức tạp mà không cần rời khỏi tập tin mã đang hoạt động.",
    "A dedicated Activity Bar view that performs background polling every 5 seconds. Includes a real-time <b>Agent Health</b> monitor tracking all 5 states (Working, Connected, Idle, Completed, Error) with native VS Code theme-aware icons and colors.":
    ...
    '📖 Full developer guide: <a href="https://github.com/billphamhypertek/agent-code-monitoring/blob/master/vscode-extension/README.md"><code>vscode-extension/README.md</code></a>':
      '📖 Hướng dẫn đầy đủ cho nhà phát triển: <a href="https://github.com/billphamhypertek/agent-code-monitoring/blob/master/vscode-extension/README.md"><code>vscode-extension/README.md</code></a>',
    "The dashboard ships as an optional <strong>native desktop application</strong> — a <code>desktop/</code> workspace that wraps the existing server and client into a macOS <code>.app</code> (distributed as a <code>.dmg</code>) and a Windows <code>.exe</code> (an NSIS installer plus a no-install portable build) you install once and forget. <code>desktop/</code> is a sibling workspace to <code>client/</code>, <code>server/</code>, <code>mcp/</code>, and <code>vscode-extension/</code>, built with <strong>Electron 35</strong>. It <strong>embeds the Express server in-process</strong> — it <code>require()</code>s <code>server/index.js</code> directly in the same Node runtime as the Electron main process (no child process, no IPC) — and renders the already-built React client in a <code>BrowserWindow</code>. Everything you see in the browser at <code>localhost:4820</code> lives inside this window, with native OS lifecycle on top.":
    ...
    '<span class="caption-icon">🍎🪟</span> <span>The full dashboard, natively on macOS <strong>and</strong> Windows — same React client, same Express server, real <code>BrowserWindow</code>. Menu-bar / notification-area (tray) icon included. Shipped as a macOS DMG and a Windows EXE (macOS shown) — see <a href="https://github.com/billphamhypertek/agent-code-monitoring/blob/master/DESKTOP.md"><code>DESKTOP.md</code></a>.</span>':
      '<span class="caption-icon">🍎🪟</span> <span>Bảng điều khiển đầy đủ, chạy gốc trên macOS <strong>và</strong> Windows — cùng một máy khách React, cùng một máy chủ Express, một <code>BrowserWindow</code> thực sự. Bao gồm biểu tượng thanh menu / khu vực thông báo (khay). Được phát hành dưới dạng DMG cho macOS và EXE cho Windows (hình minh họa là macOS) — xem <a href="https://github.com/billphamhypertek/agent-code-monitoring/blob/master/DESKTOP.md"><code>DESKTOP.md</code></a>.</span>',
    '<span class="caption-icon">🪟</span> <span>The same dashboard as a native <strong>Windows</strong> app — real <code>BrowserWindow</code> with the native Windows window menu, live Activity Feed, and the Tabby companion. A notification-area (system tray) icon sits beside the clock for quick access.</span>':
    ...
    "Launch the desktop app and <code>npm run dev</code> at the same time and both stay real-time. Each server appends its <code>{port, pid, startedAt}</code> entry to <code>~/.claude/.agent-code-monitoring.json</code> on startup; the Claude Code hook handler reads that list and fan-outs every event to every live entry in parallel. Stale entries self-evict via a PID liveness check on read, so a crashed server can't misroute events to a dead port.":
      "Khởi chạy ứng dụng máy tính để bàn và <code>npm run dev</code> cùng một lúc và cả hai đều giữ thời gian thực. Mỗi máy chủ thêm mục <code>{port, pid, startedAt}</code> của nó vào <code>~/.claude/.agent-code-monitoring.json</code> khi khởi động; trình xử lý hook của Claude Code đọc danh sách đó và phân phối song song mọi sự kiện đến mọi mục còn sống. Các mục cũ tự loại bỏ thông qua một kiểm tra tính sống của PID khi đọc, vì vậy một máy chủ bị sập không thể định tuyến sai các sự kiện đến một cổng đã chết.",
    "Double-launching the app just focuses the existing window — no second server, no port collision, on every platform. The lock is acquired via <code>requestSingleInstanceLock()</code> before any server boots.":
    ...
    'Want a build straight off the tip of <code>master</code>, ahead of the next tagged release? Every green run of the <code>🍎 macOS Desktop (DMG)</code> job on <code>macos-latest</code> uploads the universal DMG as the <code>AgentCodeMonitoring-dmg</code> workflow artifact, and the <code>🪟 Windows Desktop (EXE)</code> job on <code>windows-latest</code> uploads the installer + portable EXEs as the <code>AgentCodeMonitoring-win</code> artifact. Open the <a href="https://github.com/billphamhypertek/agent-code-monitoring/actions/workflows/ci.yml?query=branch%3Amaster+is%3Asuccess" target="_blank" rel="noopener noreferrer" class="dl-chip"> <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><circle cx="12" cy="12" r="9"/><polyline points="12 7 12 12 15 14"/></svg> latest passing run </a>, scroll to its Artifacts section, and download <code>AgentCodeMonitoring-dmg</code> or <code>AgentCodeMonitoring-win</code>. (GitHub sign-in required; 14-day retention.)':
      'Muốn một bản dựng ngay từ đỉnh của <code>master</code>, trước cả bản phát hành được gắn thẻ kế tiếp? Mỗi lần chạy thành công của job <code>🍎 macOS Desktop (DMG)</code> trên <code>macos-latest</code> đều tải lên DMG đa kiến trúc dưới dạng artifact quy trình <code>AgentCodeMonitoring-dmg</code>, còn job <code>🪟 Windows Desktop (EXE)</code> trên <code>windows-latest</code> tải lên trình cài đặt + các EXE di động dưới dạng artifact <code>AgentCodeMonitoring-win</code>. Mở <a href="https://github.com/billphamhypertek/agent-code-monitoring/actions/workflows/ci.yml?query=branch%3Amaster+is%3Asuccess" target="_blank" rel="noopener noreferrer" class="dl-chip"> <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><circle cx="12" cy="12" r="9"/><polyline points="12 7 12 12 15 14"/></svg> lần chạy gần nhất đã vượt qua </a>, cuộn tới mục Artifacts của nó, rồi tải xuống <code>AgentCodeMonitoring-dmg</code> hoặc <code>AgentCodeMonitoring-win</code>. (Cần đăng nhập GitHub; lưu giữ 14 ngày.)',
    "From the project root, after <code>git clone</code>:":
    ...
    "Run <code>AgentCodeMonitoring-Setup-&lt;ver&gt;-x64.exe</code> — a per-user NSIS install (no admin), or run the <code>*-portable.exe</code> to skip installing":
      "Chạy <code>AgentCodeMonitoring-Setup-&lt;ver&gt;-x64.exe</code> — một bản cài NSIS theo từng người dùng (không cần quản trị), hoặc chạy <code>*-portable.exe</code> để bỏ qua việc cài đặt",
    "The EXE is unsigned by default, so SmartScreen may warn — click <em>More info → Run anyway</em>":
    ...
    '<span class="caption-icon">2️⃣</span> <span>NSIS installer, step 2 — <strong>Choose Install Location</strong>: defaults to <code>%LOCALAPPDATA%\\Programs\\Agent Code Monitoring</code>, or point it anywhere.</span>':
      '<span class="caption-icon">2️⃣</span> <span>Trình cài đặt NSIS, bước 2 — <strong>Choose Install Location</strong>: mặc định là <code>%LOCALAPPDATA%\\Programs\\Agent Code Monitoring</code>, hoặc trỏ tới bất kỳ đâu.</span>',
    '<span class="caption-icon">3️⃣</span> <span>NSIS installer, step 3 — <strong>Completing Setup</strong>: click <em>Finish</em> to launch the app and drop the tray icon in the notification area.</span>':
    ...
    'The installer and portable EXE are <strong>unsigned</strong> by default — that is all the project can offer without a paid code-signing certificate. Windows <strong>SmartScreen</strong> may show <em>"Windows protected your PC"</em> the first time you run it; click <strong>More info → Run anyway</strong>. The installer lays the app down <strong>per-user</strong> under <code>%LOCALAPPDATA%\\Programs\\Agent Code Monitoring</code> (and lets you choose the install directory) and sets an <code>AppUserModelId</code> (<code>com.billphamhypertek.acm.desktop</code>) so native toast notifications are attributed correctly and the window groups under one taskbar entry.':
      "Trình cài đặt và EXE di động mặc định <strong>chưa được ký</strong> — đó là tất cả những gì dự án có thể cung cấp khi không có chứng chỉ ký mã trả phí. Windows <strong>SmartScreen</strong> có thể hiển thị <em>“Windows protected your PC”</em> lần đầu tiên bạn chạy nó; nhấp <strong>More info → Run anyway</strong>. Trình cài đặt đặt ứng dụng <strong>theo từng người dùng</strong> dưới <code>%LOCALAPPDATA%\\Programs\\Agent Code Monitoring</code> (và cho phép bạn chọn thư mục cài đặt) và thiết lập một <code>AppUserModelId</code> (<code>com.billphamhypertek.acm.desktop</code>) để các thông báo toast gốc được quy đúng nguồn và cửa sổ được nhóm dưới một mục trên thanh tác vụ.",
    "Bundle size": "Kích thước gói",
    "The DMG is roughly 80&nbsp;MB, about 250&nbsp;MB installed on disk — the standard Electron tax; the Windows installer is comparable. The app runs natively on <strong>macOS and Windows</strong>; Linux is tracked as a follow-up. Logs live at <code>~/Library/Logs/Agent Code Monitoring/desktop.log</code> on macOS or <code>%APPDATA%\\Agent Code Monitoring\\logs\\desktop.log</code> on Windows (reach them from the tray menu → <em>Show Logs</em>).":
      "DMG khoảng 80&nbsp;MB, khoảng 250&nbsp;MB khi đã cài trên đĩa — cái giá tiêu chuẩn của Electron; trình cài đặt Windows cũng tương đương. Ứng dụng chạy gốc trên <strong>macOS và Windows</strong>; Linux được theo dõi như một hạng mục tiếp theo. Nhật ký nằm ở <code>~/Library/Logs/Agent Code Monitoring/desktop.log</code> trên macOS hoặc <code>%APPDATA%\\Agent Code Monitoring\\logs\\desktop.log</code> trên Windows (truy cập chúng từ menu khay → <em>Show Logs</em>).",
    '📖 User-facing guide: <a href="https://github.com/billphamhypertek/agent-code-monitoring/blob/master/DESKTOP.md"><code>DESKTOP.md</code></a> · architecture &amp; contributor reference: <a href="https://github.com/billphamhypertek/agent-code-monitoring/blob/master/desktop/README.md"><code>desktop/README.md</code></a>':
      '📖 Hướng dẫn dành cho người dùng: <a href="https://github.com/billphamhypertek/agent-code-monitoring/blob/master/DESKTOP.md"><code>DESKTOP.md</code></a> · tài liệu tham khảo kiến trúc &amp; người đóng góp: <a href="https://github.com/billphamhypertek/agent-code-monitoring/blob/master/desktop/README.md"><code>desktop/README.md</code></a>',
    '<span class="caption-icon">⚙️</span> Settings — model pricing editor, hook installation toggle, JSON data export, session cleanup, browser notification preferences, and system info panel with DB stats':
    ...
    '<span class="caption-icon">⭐</span> <span> Enjoying the project? <a class="star-history-caption-link" href="https://github.com/billphamhypertek/agent-code-monitoring" target="_blank" rel="noopener noreferrer">Give it a star on GitHub</a> and help more builders discover it. </span>':
      '<span class="caption-icon">⭐</span> <span> Bạn thấy dự án hữu ích? <a class="star-history-caption-link" href="https://github.com/billphamhypertek/agent-code-monitoring" target="_blank" rel="noopener noreferrer">Hãy gắn sao cho nó trên GitHub</a> và giúp nhiều nhà phát triển khác biết đến nó. </span>',
    'Clears the waiting flag and promotes the main agent to <span class="status-chip chip-working"><span class="chip-dot"></span>Working</span>. The only reliable signal that text-only assistant turns have started — they emit no <code>PreToolUse</code> before <code>Stop</code>.':
    ...
    'Want a build straight off the tip of <code>master</code>, ahead of the next tagged release? Every green run of the <code>🍎 macOS Desktop (DMG)</code> job on <code>macos-latest</code> uploads the universal DMG as the <code>AgentCodeMonitoring-dmg</code> workflow artifact, and the <code>🪟 Windows Desktop (EXE)</code> job on <code>windows-latest</code> uploads the installer + portable EXEs as the <code>AgentCodeMonitoring-win</code> artifact. Open the <a href="https://github.com/billphamhypertek/agent-code-monitoring/actions/workflows/ci.yml?query=branch%3Amaster+is%3Asuccess" target="_blank" rel="noopener noreferrer" class="dl-chip"> <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><circle cx="12" cy="12" r="9"></circle><polyline points="12 7 12 12 15 14"></polyline></svg> latest passing run </a>, scroll to its Artifacts section, and download <code>AgentCodeMonitoring-dmg</code> or <code>AgentCodeMonitoring-win</code>. (GitHub sign-in required; 14-day retention.)':
      'Muốn một bản dựng ngay từ đầu nhánh <code>master</code>, trước cả bản phát hành được gắn thẻ tiếp theo? Mỗi lần chạy thành công của job <code>🍎 macOS Desktop (DMG)</code> trên <code>macos-latest</code> đều tải lên tệp DMG đa kiến trúc dưới dạng workflow artifact <code>AgentCodeMonitoring-dmg</code>, và job <code>🪟 Windows Desktop (EXE)</code> trên <code>windows-latest</code> tải lên trình cài đặt + các EXE di động dưới dạng artifact <code>AgentCodeMonitoring-win</code>. Mở <a href="https://github.com/billphamhypertek/agent-code-monitoring/actions/workflows/ci.yml?query=branch%3Amaster+is%3Asuccess" target="_blank" rel="noopener noreferrer" class="dl-chip"> <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><circle cx="12" cy="12" r="9"></circle><polyline points="12 7 12 12 15 14"></polyline></svg> latest passing run </a>, cuộn đến phần Artifacts của nó, rồi tải về <code>AgentCodeMonitoring-dmg</code> hoặc <code>AgentCodeMonitoring-win</code>. (Cần đăng nhập GitHub; lưu giữ 14 ngày.)',
