...
<a href="https://www.star-history.com/?repos=billphamhypertek%2Fagent-code-monitoring&type=date&legend=top-left">
 <picture>
   <source media="(prefers-color-scheme: dark)" srcset="https://api.star-history.com/chart?repos=billphamhypertek/agent-code-monitoring&type=date&theme=dark&legend=top-left" />
   <source media="(prefers-color-scheme: light)" srcset="https://api.star-history.com/chart?repos=billphamhypertek/agent-code-monitoring&type=date&legend=top-left" />
   <img alt="Star History Chart" src="https://api.star-history.com/chart?repos=billphamhypertek/agent-code-monitoring&type=date&legend=top-left" />
 </picture>
</a>
...
```bash
git clone https://github.com/billphamhypertek/agent-code-monitoring.git
cd agent-code-monitoring
npm run setup
```
...
Cách nhanh nhất là **tải trình cài dựng sẵn** từ [bản GitHub Release mới nhất](https://github.com/billphamhypertek/agent-code-monitoring/releases/latest) (CI tự động phát hành một `vX.Y.Z` mỗi khi `package.json` được nâng phiên bản trên `master`):

- **macOS** — tải `AgentCodeMonitoring-<version>-arm64.dmg` (Apple Silicon) hoặc `-x64.dmg` (Intel) rồi kéo **Agent Code Monitoring.app** vào `/Applications`.
- **Windows** — tải `AgentCodeMonitoring-Setup-<version>-x64.exe` (trình cài) hoặc `AgentCodeMonitoring-<version>-x64-portable.exe` (không cần cài) rồi chạy.
...
| `npm run mcp:docker:build` | Xây dựng hình ảnh vùng chứa MCP bằng Docker (`agent-code-monitoring-mcp:local`) |
| `npm run mcp:podman:build` | Xây dựng hình ảnh vùng chứa MCP bằng Podman (`localhost/agent-code-monitoring-mcp:local`) |
...
nén có thể cấu hình (`ACM_IMPORT_MAX_EXTRACT_BYTES`, mặc định 4 GB)
chặn các zip/tar/gzip-bomb. Kích thước tải lên bị giới hạn theo từng
tệp (`ACM_IMPORT_MAX_BYTES`, mặc định 1 GB) và theo yêu cầu
(`ACM_IMPORT_MAX_FILES`, mặc định 2000). Mỗi yêu cầu có thư mục tạm
...
```json
{
  ...
  "repo_root": "/Users/you/agent-code-monitoring",
...
<p align="center">
  <img src="images/macos.png" alt="Agent Code Monitoring chạy như một ứng dụng máy tính để bàn gốc" width="100%">
  <br>
...
<p align="center">
  <img src="images/windows_app.png" alt="Agent Code Monitoring chạy như một ứng dụng máy tính để bàn gốc trên Windows, hiển thị Activity Feed với thanh trình đơn cửa sổ Windows và bảng Tabby" width="100%">
  <br>
...
```mermaid
flowchart TD
    subgraph repo["agent-code-monitoring (repo root)"]
...
Tải từ [**Releases → latest**](https://github.com/billphamhypertek/agent-code-monitoring/releases/latest) (công khai, không cần đăng nhập GitHub). Mỗi khi bản cập nhật `version` trong `package.json` được đẩy lên `master`, CI tự động phát hành một `vX.Y.Z` mới, nên liên kết này luôn trỏ đến bản build hiện tại:

| Nền tảng | Tệp asset | Ghi chú |
| --- | --- | --- |
| macOS (Apple Silicon) | `AgentCodeMonitoring-<ver>-arm64.dmg` | kéo vào `/Applications` |
| macOS (Intel) | `AgentCodeMonitoring-<ver>-x64.dmg` | kéo vào `/Applications` |
| Windows (trình cài) | `AgentCodeMonitoring-Setup-<ver>-x64.exe` | cài theo từng người dùng, không cần quyền admin |
| Windows (portable) | `AgentCodeMonitoring-<ver>-x64-portable.exe` | chạy mà không cần cài |

Nếu cần **bản build theo từng commit**, dùng artifact CI (cần đăng nhập, lưu giữ 14 ngày): `AgentCodeMonitoring-dmg` từ job `🍎 macOS Desktop (DMG)` và `AgentCodeMonitoring-win` từ job `🪟 Windows Desktop (EXE)`.

Cài đặt:

- **macOS** — mở (mount) tệp `.dmg`, rồi kéo `Agent Code Monitoring.app` vào thư mục `Applications`. macOS có thể hiện cảnh báo Gatekeeper trong lần chạy đầu tiên — xem mục [Gatekeeper / SmartScreen](#gatekeeper--smartscreen-lần-chạy-đầu-tiên) bên dưới.
- **Windows** — chạy `AgentCodeMonitoring-Setup-<ver>-x64.exe`. Nó cài **theo từng người dùng** vào `%LOCALAPPDATA%\Programs\Agent Code Monitoring` (không cần nâng quyền admin) và cho bạn chọn thư mục cài; hoặc chạy `*-portable.exe` để khởi động mà không cần cài. Windows **SmartScreen** có thể hiện cảnh báo trong lần chạy đầu tiên — xem bên dưới.
...
- **Biểu tượng cửa sổ và thanh tác vụ** — `BrowserWindow` được gắn với logo ứng dụng màu (`icon.ico` trên Windows, `icon.png` ở nơi khác), nên thanh tiêu đề / thanh tác vụ hiển thị đúng biểu tượng Agent Code Monitoring — kể cả khi chạy `npm run desktop:dev` chưa đóng gói cũng không còn hiện biểu tượng Electron chung chung nữa.
...
- **Nhật ký** nằm tại `~/Library/Logs/Agent Code Monitoring/desktop.log` (macOS) hoặc `%APPDATA%\Agent Code Monitoring\logs\desktop.log` (Windows) (dùng *Show Logs* trong trình đơn để mở).
- **Dữ liệu của bạn** (cơ sở dữ liệu SQLite và khóa VAPID) nằm trong thư mục dữ liệu ứng dụng theo từng người dùng, **bên ngoài** gói ứng dụng / thư mục cài — `~/Library/Application Support/Agent Code Monitoring/data/` trên macOS, `%APPDATA%\Agent Code Monitoring\data\` trên Windows — nên dữ liệu **sống sót qua các lần cài lại và cập nhật ứng dụng**. Một gói đã đóng gói là chỉ-đọc, nên ghi cơ sở dữ liệu bên trong sẽ phá vỡ History Import và việc lưu sự kiện; lưu trong thư mục dữ liệu ứng dụng khắc phục điều này và nghĩa là lịch sử đã nhập của bạn không bị ảnh hưởng khi thay thế hoặc nâng cấp ứng dụng. (Trình gỡ cài NSIS của Windows mặc định giữ lại dữ liệu này.)
...
- Xây dựng **artifact phát hành cho mọi người** → dùng `desktop:dmg` universal và chờ lâu hơn. CI đã dựng DMG universal và tải lên dưới dạng artifact `AgentCodeMonitoring-dmg` nên bạn hiếm khi cần tự dựng.
...
`.github/workflows/ci.yml` có hai job desktop được **lọc theo đường dẫn** (một job `changes` dùng `dorny/paths-filter` phát hiện thay đổi trong `desktop/**`; các job này cũng chạy trên mọi `push`, hoặc khi PR gắn nhãn `desktop`): job `🍎 macOS Desktop (DMG)` chạy trên `macos-latest` dựng DMG universal rồi tải lên dưới dạng artifact `AgentCodeMonitoring-dmg` (hai DMG một-kiến-trúc); job `🪟 Windows Desktop (EXE)` chạy trên `windows-latest` dựng rồi tải lên dưới dạng artifact `AgentCodeMonitoring-win` (trình cài NSIS + bản portable). Khi đẩy bản nâng phiên bản lên `master`, job `release` đính kèm **cả** DMG macOS lẫn `.exe` Windows vào bản GitHub Release `vX.Y.Z` đã phát hành. Biểu tượng Windows (`desktop/assets/icon.ico`) được commit vào kho mã (tạo lại từ `icon.png` bằng `npm run build:win-icon`, dùng PowerShell + .NET, không cần công cụ phụ).
...
**Agent Code Monitoring** hiện đã có sẵn dưới dạng tiện ích mở rộng VS Code chính thức, cho phép bạn giám sát các tác nhân AI của mình mà không cần rời khỏi trình chỉnh sửa.
...
> [!TIP]
> Extension on VS Code Marketplace: [Agent Code Monitoring](https://marketplace.visualstudio.com/items?itemName=hoangsonw.agent-code-monitoring)
...
|--------|----------------|--------|
| **phân tích acm** | `claude plugin install acm-analytics@billphamhypertek-agent-code-monitoring` | `session-report`, `cost-breakdown`, `usage-trends`, `productivity-score` |
| **năng suất acm** | `claude plugin install acm-productivity@billphamhypertek-agent-code-monitoring` | `daily-standup`, `weekly-report`, `sprint-summary`, `workflow-optimizer` |
| **acm-devtools** | `claude plugin install acm-devtools@billphamhypertek-agent-code-monitoring` | `session-debug`, `hook-diagnostics`, `data-export`, `health-check` |
| **thông tin chi tiết về acm** | `claude plugin install acm-insights@billphamhypertek-agent-code-monitoring` | `pattern-detect`, `anomaly-alert`, `optimization-suggest`, `session-compare` |
| **bảng điều khiển acm** | `claude plugin install acm-dashboard@billphamhypertek-agent-code-monitoring` | `dashboard-status`, `quick-stats` + máy chủ MCP |

### Bao gồm các công cụ CLI

- `acm-stats` — Bảng điều khiển thiết bị đầu cuối (phiên, chi phí, mã thông báo có đường cơ sở nén)
- `acm-doctor` — Chẩn đoán hệ thống (API, cơ sở dữ liệu, hook, làm mới dữ liệu)
- `acm-export` — Xuất dữ liệu (JSON, CSV) cho phiên, sự kiện, phân tích, chi phí

### Cách sử dụng ví dụ

```bash
# In Claude Code, after installing a plugin:
/acm-analytics:session-report latest
/acm-analytics:cost-breakdown this week
/acm-productivity:daily-standup today
/acm-insights:pattern-detect tools
/acm-dashboard:quick-stats
```
...
```
Sonnet 4.6 | nguyens6 | ~/agent-code-monitoring/client | main | ████████░░ 79% | 3↑ 2↓ 156586c
```
...
| CWD         | Màu vàng               | `~/agent-code-monitoring` |
...
|-- plugins/
|   |-- acm-analytics/         # Analytics: session reports, cost breakdown, usage trends, productivity score
...
|   |   +-- bin/acm-stats      # Terminal dashboard CLI
|   |-- acm-productivity/      # Productivity: standups, reports, sprints, workflow optimizer
|   |-- acm-devtools/          # DevTools: debug, diagnostics, export, health checks
|   |   +-- bin/                # acm-doctor + acm-export CLIs
|   |-- acm-insights/          # Insights: patterns, anomalies, optimization, comparison
|   +-- acm-dashboard/         # Dashboard connector: status, quick stats, MCP integration
...
| Lần chạy đầu macOS hiện "Apple could not verify…" | DMG được ký ad-hoc theo mặc định. Chạy `xattr -cr "/Applications/Agent Code Monitoring.app"`, hoặc nhấp *Open Anyway* trong *System Settings → Privacy & Security* |
...
| Lịch sử đã nhập biến mất sau khi cài lại/cập nhật ứng dụng máy tính để bàn | Các bản dựng cũ lưu cơ sở dữ liệu bên trong gói ứng dụng có thể bị thay thế. Lỗi này đã được sửa — dữ liệu nay nằm tại `~/Library/Application Support/Agent Code Monitoring/data/` (macOS) hoặc `%APPDATA%\Agent Code Monitoring\data\` (Windows), bên ngoài gói và sống sót qua các lần cài lại. Sau khi nâng cấp từ bản dựng trước khi sửa lỗi, hãy chạy lại **Import History → Rescan** một lần |
