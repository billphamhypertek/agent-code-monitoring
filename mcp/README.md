# Agent Code Monitoring MCP Server
...
```bash
# Docker (repo root)
docker build -f mcp/Dockerfile -t agent-code-monitoring-mcp:local .

# Podman (repo root)
podman build -f mcp/Dockerfile -t localhost/agent-code-monitoring-mcp:local .
```
...
docker run --rm -i --init \
  ...
  agent-code-monitoring-mcp:local
...
podman run --rm -i --network=host \
  ...
  localhost/agent-code-monitoring-mcp:local
...
```json
{
  "mcpServers": {
    "agent-code-monitoring": {
      "command": "node",
      "args": [
        "C:\\ABSOLUTE\\PATH\\TO\\agent-code-monitoring\\mcp\\build\\index.js"
      ],
      "env": {
...
| --- | --- | --- |
| `MCP_SERVER_NAME` | `agent-code-monitoring-mcp` | MCP server name reported to host |
