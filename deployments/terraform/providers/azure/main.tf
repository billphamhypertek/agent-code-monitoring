# ─────────────────────────────────────────────────────────────────────────────
# Azure Provider – Full implementation for Agent Code Monitoring
#
# Architecture:
...
locals {
  ...
  storage_account_name = lower(replace(substr("acm${var.environment}${substr(md5(var.project_name), 0, 8)}", 0, 24), "-", ""))
  ...
  common_tags = merge(
    {
      ...
      repository     = "agent-code-monitoring"
    },
    var.tags,
  )
}
