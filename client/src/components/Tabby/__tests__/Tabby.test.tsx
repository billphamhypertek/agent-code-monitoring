...
describe("Tabby widget", () => {
  ...
  it("respects the enabled preference", () => {
    localStorage.setItem("agent-code-monitoring-tabby-enabled", "false");
    renderTabby();
  ...
  it("dragging snaps to an edge, persists position, and does not open the panel", () => {
    ...
    fireEvent.click(btn);
    expect(screen.queryByRole("dialog")).not.toBeInTheDocument();
    const saved = JSON.parse(localStorage.getItem("agent-code-monitoring-tabby-pos") || "{}");
  ...
  it("a sub-threshold pointer move is treated as a tap, not a drag", () => {
    ...
    expect(localStorage.getItem("agent-code-monitoring-tabby-pos")).toBeNull();
  });

  it("restores a persisted left-edge position on mount", () => {
    localStorage.setItem("agent-code-monitoring-tabby-pos", JSON.stringify({ side: "left", y: 0.2 }));
    renderTabby();
