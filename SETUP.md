...
A comprehensive guide to setting up and configuring the Agent Code Monitoring, including how it integrates with Claude Code, environment variables, container deployment, and troubleshooting common issues.

## How it works

Agent Code Monitoring integrates with Claude Code through its native hook system. When Claude Code performs any action (session start, tool use, turn completion, subagent finish, session exit), it fires a hook that calls a small Node.js script bundled with this project. That script forwards the event over HTTP to the dashboard server, which stores it in SQLite and broadcasts it to the browser over WebSocket.
...
| `ACM_IMPORT_MAX_BYTES` | `1073741824` (1 GB) | Maximum size per uploaded file on `/api/import/upload` |
| `ACM_IMPORT_MAX_FILES` | `2000` | Maximum number of files per upload request |
| `ACM_IMPORT_MAX_EXTRACT_BYTES` | `4294967296` (4 GB) | Maximum uncompressed bytes any single archive is allowed to expand to (zip-bomb defense) |
...
The **Agent Code Monitoring** is available as an integrated VS Code extension for seamless monitoring within your editor.
...
`ACM_IMPORT_MAX_BYTES`, `ACM_IMPORT_MAX_FILES`, and
`ACM_IMPORT_MAX_EXTRACT_BYTES`.
