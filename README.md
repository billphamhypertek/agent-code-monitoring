# Agent Code Monitoring for Claude Code
...
| **Plugin Marketplace**             | Official Claude Code plugin marketplace with 5 plugins (acm-analytics, acm-productivity, acm-devtools, acm-insights, acm-dashboard). 18 skills, 4 agents, 3 CLI tools, 2 hook configs. All grounded in actual data model — token baselines, pricing engine, workflow intelligence (11 datasets), session metadata. Install via `claude plugin marketplace add` |
...
| `npm run mcp:docker:build` | Build MCP container image with Docker (`agent-code-monitoring-mcp:local`) |
| `npm run mcp:podman:build` | Build MCP container image with Podman (`localhost/agent-code-monitoring-mcp:local`) |
...
configurable extraction cap (`ACM_IMPORT_MAX_EXTRACT_BYTES`, default
4 GB) stops zip/tar/gzip bombs. Upload size is capped per file
(`ACM_IMPORT_MAX_BYTES`, default 1 GB) and per request
(`ACM_IMPORT_MAX_FILES`, default 2000). All staging directories are
...
The **Agent Code Monitoring** is available as a first-class VS Code extension, allowing you to monitor your AI agents without leaving your editor.
...
**Option A — download a pre-built installer (recommended).** From **[Releases → latest](https://github.com/billphamhypertek/agent-code-monitoring/releases/latest)** (public, no GitHub sign-in). CI auto-publishes a new `vX.Y.Z` release whenever the version in `package.json` is bumped on `master`, so this link always serves the current build:
...
|--------|----------------|--------|
| **acm-analytics** | `claude plugin install acm-analytics@billphamhypertek-agent-code-monitoring` | `session-report`, `cost-breakdown`, `usage-trends`, `productivity-score` |
| **acm-productivity** | `claude plugin install acm-productivity@billphamhypertek-agent-code-monitoring` | `daily-standup`, `weekly-report`, `sprint-summary`, `workflow-optimizer` |
...
| **acm-insights** | `claude plugin install acm-insights@billphamhypertek-agent-code-monitoring` | `pattern-detect`, `anomaly-alert`, `optimization-suggest`, `session-compare` |
| **acm-dashboard** | `claude plugin install acm-dashboard@billphamhypertek-agent-code-monitoring` | `dashboard-status`, `quick-stats` + MCP server |

### CLI tools included

- `acm-stats` — Terminal dashboard (sessions, costs, tokens with compaction baselines)
- `acm-doctor` — System diagnostics (API, database, hooks, data freshness)
- `acm-export` — Data export (JSON, CSV) for sessions, events, analytics, costs
...
```
nguyens6@host ~/agent-code-monitoring/client | Sonnet 4.6 | main | ████████░░ 79% | 3↑ 2↓ 156586c | $0.4231
```
...
| User        | Green                | `nguyens6`                                                     |
| CWD         | Yellow               | `~/agent-code-monitoring`                                            |
...
All contributors must sign the [Contributor License Agreement](https://github.com/billphamhypertek/agent-code-monitoring/blob/master/CLA.md). This is enforced automatically on every pull request by the `🖋️ CLA Assistant` GitHub Action: the first time you open a PR, a bot asks you to sign by commenting `I have read the CLA Document and I hereby sign the CLA`. The PR's **CLA Assistant** status check stays red until you do, and signing once covers all future contributions.
