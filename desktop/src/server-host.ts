 ...
 *   - `ACM_DESKTOP_BIND_PORT`: bind exactly this port (no adoption, no fallback).
 *     Used by the smoke test to verify the spawned process actually started a
 *     server rather than finding an unrelated one.
 *   - `ACM_DESKTOP_NO_ADOPT=1`: skip the "is there already a healthy server
 *     on 4820?" check and always start our own.
 */
export async function startEmbeddedServer(): Promise<ServerHandle> {
  const forcedPort = process.env.ACM_DESKTOP_BIND_PORT
    ? parseInt(process.env.ACM_DESKTOP_BIND_PORT, 10)
    : null;
  const noAdopt = process.env.ACM_DESKTOP_NO_ADOPT === "1" || forcedPort !== null;
  ...
  // transcript snapshots) to the shared user-global `~/.claude/agent-code-monitoring/`
