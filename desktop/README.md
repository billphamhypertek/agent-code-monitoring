...
The **desktop workspace** ships the Agent Code Monitoring dashboard as a
...
On startup the server writes its live port to `~/.claude/.agent-code-monitoring.json`
...
sequenceDiagram
    ...
    T->>E: spawn with ACM_DESKTOP_BIND_PORT=<port>
...
`ACM_DESKTOP_BIND_PORT` forces the server onto an exact port (no adoption, no
...
|---|---|---|
| `ACM_DESKTOP_BIND_PORT` | `server-host.ts` | Bind exactly this port — disables adoption and fallback. Used by the smoke test. |
| `ACM_DESKTOP_NO_ADOPT` | `server-host.ts` | `=1` → never adopt an existing `:4820` server; always start our own. |
| `ACM_DESKTOP_VERBOSE` | `logger.ts` | Mirror `info`/`warn` log lines to stdout (errors always go to stderr). |
...
- **`server/lib/server-info.js`** *(new)* — multi-server discovery file at
  `~/.claude/.agent-code-monitoring.json`. Every running dashboard appends its
