...
// `ACM_DESKTOP_BIND_PORT` tells the desktop process to bind exactly this port,
...
describe("desktop smoke", () => {
  before(async () => {
    electronProc = spawn(ELECTRON_BIN, [MAIN_JS], {
      ...
      env: {
        ...
        ACM_DESKTOP_VERBOSE: "1",
        ACM_DESKTOP_BIND_PORT: String(TEST_PORT),
      },
  ...
  it("brings up the embedded server and serves /api/health on the bound port", async () => {
    ...
    assert.equal(
      port,
      TEST_PORT,
      `desktop process should have bound ACM_DESKTOP_BIND_PORT=${TEST_PORT}`
    );
    assert.ok(
