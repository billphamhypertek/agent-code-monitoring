...
Agent Code Monitoring is a local-first monitoring platform for Claude Code sessions. It captures agent lifecycle events via Claude Code's native hook system, persists them in SQLite, and presents them through a React dashboard with real-time WebSocket updates.
...
C4Context
    ...
    System(dashboard, "Agent Code Monitoring", "Monitoring platform")
...
| `server/routes/import.js`   | Express router for the Import History feature. Three endpoints funnel into the same pipeline: `POST /api/import/rescan` (default projects dir), `POST /api/import/scan-path` (arbitrary absolute dir with `~` expansion), `POST /api/import/upload` (multer multipart accepting `.jsonl`, `.meta.json`, `.zip`, `.tar`, `.tar.gz`, `.tgz`, `.gz`). `GET /api/import/guide` returns OS-aware instructions + archive command + default-dir stats. Each request uses a per-request temp dir (`req._acmUploadDir` for multer staging, a separate `workDir` for extraction) that is reclaimed in `finally`. Progress is broadcast as `import.progress` websocket messages throttled at ~150 ms. Limits configurable via `ACM_IMPORT_MAX_BYTES` / `ACM_IMPORT_MAX_FILES` |
| `server/lib/archive.js`     | Safe archive extraction: `.zip` via `adm-zip`, `.tar`/`.tar.gz`/`.tgz` via `tar`, plain `.gz` via `zlib` in streaming mode. Every entry is validated through `safeJoin` which rejects absolute paths and `..` traversal before any bytes are written. Enforces a hard extraction cap (`MAX_EXTRACT_BYTES`, default 4 GB, tunable via `ACM_IMPORT_MAX_EXTRACT_BYTES`) with `ExtractionLimitError` surfaced as HTTP 413 from the upload route — defense against zip/tar/gzip bombs. Also provides `detectKind` for filename-based dispatch and `mkTempDir`/`rmTempDir` helpers |
...
- Resolution order is **env override → discovery file → default**:
  ...
  - Otherwise `server/lib/server-info.js` reads `~/.claude/.agent-code-monitoring.json`, prunes dead-PID entries, and returns every live `port` for the handler to POST to in parallel.
...
sequenceDiagram
    ...
    M->>M: mkTempDir('acm-upload-*')<br/>stored on req._acmUploadDir
    M->>M: fileFilter: reject unsupported<br/>(tracked in req._acmRejected)
    alt All files rejected
      API-->>UI: 400 NO_FILES<br/>+ rejected_files[]
    else Files accepted
      API->>AR: mkTempDir('acm-import-work-*')
    ...
    API->>M: rmTempDir(req._acmUploadDir)
```
...
| Concurrent upload temp-dir collisions       | Per-request temp dir on `req._acmUploadDir`; created in multer `destination`, cleaned in `finally`  |
...
| --------------------------------- | ----------- | ----------------------------------------------------------------- |
| `ACM_IMPORT_MAX_BYTES`           | 1 GB        | Maximum size per uploaded file                                    |
| `ACM_IMPORT_MAX_FILES`           | 2000        | Maximum files per upload request                                  |
| `ACM_IMPORT_MAX_EXTRACT_BYTES`   | 4 GB        | Ceiling on total uncompressed bytes from any single archive       |
...
```json
{
  ...
  "data": {
    ...
    "processed": 184,
    "total": 512,
    "current": "/tmp/acm-import-work-xyz/project/<uuid>.jsonl",
...
- `plugins/` provides distributable plugin marketplace (see [Plugin Marketplace](#plugin-marketplace)):
  - acm-analytics (session reports, cost analysis, usage trends, productivity scoring)
  - acm-productivity (standups, weekly reports, sprint summaries, workflow optimization)
  - acm-devtools (session debugging, hook diagnostics, data export, health checks)
  - acm-insights (pattern detection, anomaly alerting, optimization, session comparison)
  - acm-dashboard (status checks, quick stats, MCP integration)
...
graph TD
    ...
    subgraph Plugins["plugins/"]
        A["acm-analytics<br/>4 skills, 1 agent, 1 CLI"]
        P["acm-productivity<br/>4 skills, 1 agent"]
        D["acm-devtools<br/>4 skills, 1 agent, 2 CLIs"]
        I["acm-insights<br/>4 skills, 1 agent"]
        C["acm-dashboard<br/>2 skills, MCP config"]
    end
...
Skills are namespaced: `/acm-analytics:session-report`, `/acm-productivity:daily-standup`, etc.

### Plugin Catalog

| Plugin | Skills | Agent | CLI Tools | Hooks |
|--------|--------|-------|-----------|-------|
| **acm-analytics** | `session-report`, `cost-breakdown`, `usage-trends`, `productivity-score` | `analytics-advisor` | `acm-stats` | Stop, SubagentStop |
| **acm-productivity** | `daily-standup`, `weekly-report`, `sprint-summary`, `workflow-optimizer` | `productivity-coach` | — | SessionStart, SessionEnd |
| **acm-devtools** | `session-debug`, `hook-diagnostics`, `data-export`, `health-check` | `issue-triager` | `acm-doctor`, `acm-export` | — |
| **acm-insights** | `pattern-detect`, `anomaly-alert`, `optimization-suggest`, `session-compare` | `insights-advisor` | — | — |
| **acm-dashboard** | `dashboard-status`, `quick-stats` | — | — | — |
...
The **Agent Code Monitoring** VS Code extension provides an integrated monitoring experience directly within the editor. It communicates with the local dashboard server via standard HTTP APIs and renders the dashboard UI in a webview.
...
3. **Deep Linking**: Commands like `agent-code-monitoring.openDashboard` accept arguments (e.g., a session ID or page path like `analytics`) to route the webview to specific views within the React SPA.
...
| **`logger.ts`** | File logger to `~/Library/Logs/Agent Code Monitoring/desktop.log` (macOS) or `%APPDATA%\Agent Code Monitoring\logs\desktop.log` (Windows) -- the main process has no console when launched from Finder / Explorer. |
| **`constants.ts`** | Shared identifiers, including the `APP_ID` (`com.billphamhypertek.acm.desktop`) that `main.ts` sets as the Windows AppUserModelId. |
...
```mermaid
flowchart TD
    start["startEmbeddedServer()"] --> forced{"ACM_DESKTOP_BIND_PORT set?"}
...
Port preference order is **4820 → 4821–4829 → a random port in 49152–49500**. Two environment overrides exist primarily for testing: `ACM_DESKTOP_BIND_PORT` binds an exact port (disabling adoption and fallback, used by the smoke test), and `ACM_DESKTOP_NO_ADOPT=1` always starts a fresh server. Before `require()`ing the server module, the host sets `NODE_ENV=production`, `DASHBOARD_PORT=<port>`, and `DASHBOARD_DATA_DIR=<userData>/data` (see [Writable Data Directory](#writable-data-directory) below) so the server reads them from `process.env`.
...
sequenceDiagram
    ...
    alt launched at login (--acm-hidden / wasOpenedAtLogin)
...
| **Launched at login** | The dashboard window is **not** shown -- only the tray icon. Detected via macOS `wasOpenedAtLogin` (dock hidden, `openAsHidden`) or, on Windows, the `--acm-hidden` arg written into the `HKCU\…\Run` startup command. |
...
`electron-builder` produces `Agent Code Monitoring.app` on macOS and `Agent Code Monitoring.exe` (NSIS installer + portable) on Windows. On both platforms the Electron main-process code is compiled (`tsc` → `out/`) and packed into `app.asar`; the rest of the repo is shipped as **`extraResources`** -- plain files under the bundle's `Resources/app/` (macOS) or the install dir's `resources\app\` (Windows). The internal layout is the same shape on both:

```mermaid
flowchart TD
    appbundle["Agent Code Monitoring.app (macOS)<br/>Agent Code Monitoring install dir (Windows)"]
    appbundle --> contents["Contents/ (macOS)<br/>install root (Windows)"]
    contents --> macos["MacOS/ — Electron binary (macOS)<br/>Agent Code Monitoring.exe (Windows)"]
...
```mermaid
graph LR
    LAUNCH["Open Agent Code Monitoring<br/>(.app / .exe)"] --> MAIN["Electron main process<br/>(Node 22 / Electron 35)"]
