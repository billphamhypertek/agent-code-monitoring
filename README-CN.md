# Claude Code Agent Code Monitoring（Agent 监控面板）
...
| **插件市场** | 官方 Claude Code 插件市场，包含 5 个插件（acm-analytics、acm-productivity、acm-devtools、acm-insights、acm-dashboard）。18 个技能、4 个 Agent、3 个 CLI 工具、2 个 Hook 配置。全部基于实际数据模型 — Token 基线、定价引擎、工作流智能（11 个数据集）、会话元数据。通过 `claude plugin marketplace add` 安装 |
...
| `npm run mcp:docker:build` | 用 Docker 构建 MCP 容器镜像（`agent-code-monitoring-mcp:local`） |
| `npm run mcp:podman:build` | 用 Podman 构建 MCP 容器镜像（`localhost/agent-code-monitoring-mcp:local`） |
...
路径段会被拒绝）。可配置的解压尺寸上限
（`ACM_IMPORT_MAX_EXTRACT_BYTES`，默认 4 GB）可阻止 zip/tar/gzip
炸弹。上传大小按单文件（`ACM_IMPORT_MAX_BYTES`，默认 1 GB）与单次
请求（`ACM_IMPORT_MAX_FILES`，默认 2000）分别限制。每个请求使用
...
**Agent Code Monitoring** 现已作为官方 VS Code 扩展提供，让你无需离开编辑器即可监控 AI Agent。
...
[`#144`](https://github.com/billphamhypertek/agent-code-monitoring/pull/144) 引入的 PWA 让 Dashboard 可以在 Chromium 系浏览器中安装，适合已经让服务器常驻运行的用户。桌面应用解决的是另一个正交问题：**无需终端窗口即可启动并保持服务器运行**。
...
从 [**Releases → latest**](https://github.com/billphamhypertek/agent-code-monitoring/releases/latest) 下载（公开，无需登录 GitHub）。每当 `master` 上的 `package.json` 版本号被提升时，CI 都会自动发布一个新的 `vX.Y.Z`，因此该链接始终指向当前版本：
...
| **acm-dashboard** | `claude plugin install acm-dashboard@billphamhypertek-agent-code-monitoring` | `dashboard-status`、`quick-stats` + MCP 服务器 |

### 包含的 CLI 工具

- `acm-stats` — 终端 Dashboard（会话、成本、Token 含压缩基线）
- `acm-doctor` — 系统诊断（API、数据库、Hook、数据新鲜度）
- `acm-export` — 数据导出（JSON、CSV）用于会话、事件、分析、成本
...
|-- plugins/
|   |-- acm-analytics/         # 分析：会话报告、成本明细、使用趋势、生产力评分
...
|   |   +-- bin/acm-stats      # 终端 Dashboard CLI
|   |-- acm-productivity/      # 生产力：站会、报告、冲刺、工作流优化
|   |-- acm-devtools/          # 开发工具：调试、诊断、导出、健康检查
|   |   +-- bin/                # acm-doctor + acm-export CLI
|   |-- acm-insights/          # 洞察：模式、异常、优化、比较
|   +-- acm-dashboard/         # Dashboard 连接器：状态、快速统计、MCP 集成
|       +-- .mcp.json           # MCP 服务器配置
|-- server/
